...
void MCExtractor::Extract(const RTSState &s, const Preload_Train &preload, PlayerId player_id, bool respect_fow, GameState *gs) {
   ...
   auto & s_enemy_mask = gs->s_enemy_mask;
   s_enemy_mask.resize(num_enemy * num_tower);
   std::fill(s_enemy_mask.begin(),s_enemy_mask.end(),0.0f);

   extract_enemy_and_mask(env,player_id,preload,s_enemy,s_enemy_mask);
   //printf("New Extractor FInish\n");
}
...
void MCExtractor::extract_radar(const GameEnv &env, const Preload_Train &preload,std::vector<float>& s_radar){
    ...
    for(int i =0;i<radar.size();++i){  //在我们的设计中，最大雷达数量不应该超过我们设置的数量(我们需要掌握每个雷达的情况)
        ...
        if(u_radar != nullptr){
            ...
            s_radar[offset+3] = u_radar->GetProperty()._vis_r;
            float d_t_b = preload.DistanceToBase(u_radar->GetPointF() );
            PointF v_t_b = preload.VectorToBase(u_radar->GetPointF() );
            s_radar[offset+4] = v_t_b.x;  // dx
            s_radar[offset+5] = v_t_b.y;  // dy
            s_radar[offset+6] = d_t_b;  // distance_to_base
            //朝向
            s_radar[offset+7] = u_radar->GetProperty().towards.x;
            s_radar[offset+8] = u_radar->GetProperty().towards.y;
            ++count;
        }
    }
}
...
void MCExtractor::extract_tower(Tick tick,const Preload_Train &preload,std::vector<float>& s_tower){
    ...
    for(int i =0;i<tower.size();++i){  //在我们的设计中，最大防御塔数量不应该超过我们设置的数量(我们需要掌握每个防御塔的情况)
        ...
        if(u_tower != nullptr){
            ...
            s_tower[offset+4] = v_t_b.x;  // dx
            s_tower[offset+5] = v_t_b.y;  // dy
...
//             s_enemy[offset+7] = d_t_b;  // 统计敌人在多少防御塔的射程呢s   
//             ++count;
//         }
//     }
...
void MCExtractor::extract_enemy_and_mask(const GameEnv &env, PlayerId player_id, const Preload_Train &preload,std::vector<float>& s_enemy,std::vector<float>& s_enemy_mask){
    if(s_enemy.size() != num_enemy*enemy_size ){
        printf("Wrong size of enemy\n"); 
        return;
    }
    if(s_enemy_mask.size() != num_enemy*num_tower){
        printf("Wrong size of enemy\n"); 
        return;
    }
    ...
    int count = 0;  //计数
    int tower_count = 0; // 计算敌人在射程内的塔
    int offset_1 = 0;  // s_enemy 的 offset
    int offset_2 = 0;  // s_enemy_mask 的 offset
    for(int i =0;i<enemy.size();++i){
        if(count>=num_enemy) break;  //最大可以记录num_enemy个敌人的信息
        const Unit* u_enemy = enemy[i].second;
        if(u_enemy != nullptr){
            //player.isUnitLocked(_target)
            offset_1 = count * enemy_size;
            offset_2 = count * num_tower;
            s_enemy[offset_1] = u_enemy->GetPointF().x;
            s_enemy[offset_1+1] = u_enemy->GetPointF().y;
            s_enemy[offset_1+2] = u_enemy->GetUnitType();
            s_enemy[offset_1+3] = env.isUnitLock(player_id,u_enemy->GetId())?1:0; // 是否已经被锁定
            // vector_to_base
            float d_t_b = enemy[i].first;
            PointF v_t_b = preload.VectorToBase(u_enemy->GetPointF() );
            s_enemy[offset_1+4] = v_t_b.x;  // dx
            s_enemy[offset_1+5] = v_t_b.y;  // dy
            s_enemy[offset_1+6] = d_t_b;  // distance_to_base
            tower_count = 0;
            //printf("2\n");
            if(!tower.empty()){
                 for(int j=0;j<tower.size();++j){
                     float distance_to_tower = PointF::L2Sqr(u_enemy->GetPointF(),tower[j]->GetPointF());
                     //printf("distance_to_tower %f\n",j,distance_to_tower);
                     float range = tower[j]->GetProperty()._att_r * tower[j]->GetProperty()._att_r;
                     //printf("range %f\n",range);
                     if(distance_to_tower < range){
                         s_enemy_mask[offset_2+j] =  1;
                         ++tower_count;
                     }
                
                 }
            }
            s_enemy[offset_1+7] = tower_count;  // 统计敌人在多少防御塔的射程呢
            s_enemy[offset_1+8] = u_enemy->GetProperty().times_attack; //敌人被攻击的次数
...
// void MCExtractor::extract_enemy_mask(const Preload_Train &preload,std::vector<float> &s_enemy_mask){
//     if(s_enemy_mask.size() != num_enemy*num_tower){
//         printf("Wrong size of enemy\n"); 
//         return;
//     }
...
//         const Unit* u_enemy = enemy[i].second;
//         if(u_enemy != nullptr){
//             offset = count * num_tower;
//             for(int j=0;j<tower_size;++j){
//                 float distance_to_tower = PointF::L2Sqr(u_enemy->GetPointF(),tower[j]->GetPointF());
//                 float range = tower[j]->GetProperty()._att_r * tower[j]->GetProperty()._att_r;
//                 s_enemy_mask[offset+j] =  distance_to_tower < range ? 1:0;
//             }
//             ++count;
//         }
//     }
...
PointF Preload_Train::VectorToBase(const PointF& u_loc) const{
    ...
    PointF v_t_b = PointF(_base_loc.x - u_loc.x, _base_loc.y - u_loc.y);
    v_t_b.Normalize();
    return v_t_b;
}
