...
class DataProcess(Model):
    ...
    def forward(self,x):
        # import pdb
        # pdb.set_trace()
        radar,_ = self.radarProcesser(self._var(x["s_radar"])) # 1 x 100
        tower,tower_embedding = self.towerProcesser(self._var(x["s_tower"])) # 1 x 100  20 x 100
        enemy,enemy_embedding = self.enemyProcesser(self._var(x["s_enemy"])) # 1 x 100  20 x 100
        base = self._var(x["s_base"]).view(-1,1,3)
        base,_ = self.baseProcesser(base)  # 1 x 50
        glo = self._var(x["s_global"]).view(-1,1,3) # 1 x 3
        # glo = F.relu(self.FC_2(glo)) #1 x 6
        global_stat = torch.cat([glo,base,radar,tower,enemy],2)  # 1 x 353
        # import pdb
        # pdb.set_trace()
        global_stat = F.relu(self.FC(global_stat))
        global_stat = global_stat.view(-1,1,64)
        # global_stat = global_stat.view(-1,256)
...
class TowerAttention(Model):
    ...
    def forward(self,x,target_embdding,target_mask,tower_embedding,tower_raw,use_mask = False):
    #def forward(self,x,tower_embedding,tower_raw,use_mask = False):
        ...
        if (use_mask):
            ...
            ammo = torch.split(tower_raw,1,2)[2].contiguous().view(-1,tower_raw.shape[1])
            ammo_mask = ammo
            # import pdb
            # pdb.set_trace()
            ...
            attention = attention.masked_fill(attn_mask, -1000)  # 给需要mask的地方设置一个负无穷。
        #attention = F.softmax(attention)
        return attention
