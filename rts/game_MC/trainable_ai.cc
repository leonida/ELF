...
bool TrainedAI::handle_response(const State &s, const Data &data, RTSMCAction *a) {
                   ...
                   // printf("Handle response ct = %d select %d attack %d\n",gs.ct[i],gs.uloc[i],gs.tloc[i]);
                    ...
                    if(gs.ct[i]>0){   //执行攻击命令
                       ...
                       if(enemyNums>0 && towerNums>0){
                        ...
                        //   if(u_tower != nullptr && u_enemy != nullptr)
                       // printf("防御塔 %d 向 敌人 %d  发射了 %d 发导弹 载弹量：%d 距离：%f\n",tower_id,target_id,gs.ct[i],u_tower->GetProperty().round,PointF::L2Sqr(u_tower->GetPointF(),u_enemy->GetPointF()) );
   ...
   // printf("handle respoonse finish\n");
    return true;
}
