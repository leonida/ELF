...
class Loader(CommonLoader):
    ...
    def _get_actor_spec(self): #d=定义用于actor的batch字典
        ...
        return dict(
            ...
            input=dict(T=1, keys=set(["last_r", "terminal","s_global","s_base","s_radar","s_tower","s_enemy","s_enemy_mask"])),  # 期待收到 s last_r terminal
            reply=dict(T=1, keys=set(reply_keys + self._unit_action_keys())),
        )

    def _get_train_spec(self):
       # keys = ["s", "last_r", "V", "terminal", "pi", "a"]
        keys = [ "last_r", "V", "terminal","s_global","s_base","s_radar","s_tower","s_enemy","s_enemy_mask"]
        return dict(
            batchsize=self.args.batchsize,
