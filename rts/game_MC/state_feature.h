...
private:
    ...
    static void extract_enemy_mask(const Preload_Train &preload,std::vector<float> &s_enemy_mask);
    static void extract_enemy_and_mask(const GameEnv &env, PlayerId player_id, const Preload_Train &preload,std::vector<float> &s_enemy,std::vector<float> &s_enemy_mask);
};
