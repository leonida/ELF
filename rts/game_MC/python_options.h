...
struct GameState {
    ...
    void Init(int iid, int num_action, int num_max_cmd, int mapx, int mapy, int num_ct, int num_units, int reduced_size) {
        ...
        uloc_prob.resize(num_max_cmd * num_tower, 0.0);
        tloc_prob.resize(num_max_cmd * num_tower, 0.0);
        bt_prob.resize(num_max_cmd * num_units, 0.0);
        ct_prob.resize(num_max_cmd * num_ct, 0.0);
    ...
    //    std::cout<<"enemy size "<<s_enemy.size()<<std::endl;
    //    std::cout<<"tower size "<<s_tower.size()<<std::endl;
    ...
    DECLARE_FIELD(GameState, id, a,V,last_r, s, rv, terminal, seq, game_counter, last_terminal, uloc, tloc, ct, uloc_prob, tloc_prob, ct_prob, reduced_s, reduced_next_s, action_type, s_global, s_base, s_enemy,s_enemy_mask,s_radar,s_tower);
