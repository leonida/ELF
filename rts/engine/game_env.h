...
public:
    ...
    // 基于规则分配雷达的方法
    UnitId SelectRadar_DistanceFirst(Targets& radar_info, PlayerId player_id,UnitId _target);

    void UpdateTargets(PlayerId player_id);  // 更新锁定名单
    bool Lock(PlayerId player_id,UnitId radar_id,UnitId target_id); //指定雷达锁定目标
    bool Lock(PlayerId player_id,UnitId target_id); //锁定目标，按照规则分配雷达
    ...
    void InitRadar(PlayerId player_id);
