...
class Unit;
struct GamePlan{
    int _round = 0;  // 一共有多少波敌人
    bool isInit = false; // 是否已经初始化 一般我们只初始化一次
    vector<int> start_tick; //各个批次敌人的入场时间
    map<string,vector<int> > plan; //每个批次的敌人类型和数量
    void InitPlan(int round){
            _round = round; start_tick = vector<int>(_round);
           // A B C 三类 的入场时间
           plan["A"] = vector<int>(_round);
           plan["B"] = vector<int>(_round);
           plan["C"] = vector<int>(_round);
           isInit = true;
        //    printf("InitPlan\n");
    }

    // void PrintInfo(){
    //     for(int i=0;i<start_tick.size();++i)
    //      printf("tick: %d\n",start_tick[i]);
...
public:
    ...
    // void SetEnemyRound(int round); // 设置敌人波数
    // int GetEnemyRound() const {return enemy_round;}  
    // void ChangeCurrentRound(int change){cur_round += change;}
    // int GetCurrentRound() const{return cur_round;}
    // GamePlan &GetPlan(){return _plan;}


    void ChangeBaseHit(int change){base_hit += change;}
    void ChangeTowerLoss(int change){ tower_loss += change;}
    void ChangeRadarLoss(int change){radar_loss+= change;}
    void ChangeAmmoLoss(int change){ammo_loss += change;}
    void ChangeEnemyA(int change){enemy_A += change;}
    void ChangeEnemyB(int change){enemy_B += change;}
    void ChangeEnemyC(int change){enemy_C += change;}
    void ChangeEnemyBullet(int change){enemy_Bullet += change;}
    void PrintLoss() const {
        printf("base hit:  %d  tower_loss: %d radar_loss: %d  ammo_loss: %d A: %d  B: %d  C: %d Bullet: %d\n",base_hit,tower_loss,radar_loss,ammo_loss,enemy_A,enemy_B,enemy_C,enemy_Bullet);
    }
    ...
    void FinishInitRadar(){isRadarInit = true;}
    bool IsRadarInit(){return isRadarInit;}
